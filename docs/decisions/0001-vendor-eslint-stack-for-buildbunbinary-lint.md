...
Issue amarbel-llc/bun#7 introduces an `n/no-process-exit` lint pass that runs against every entrypoint passed to `buildBunBinary` / `buildBunBinaries` at nix build time. The lint stack (`eslint`, `eslint-plugin-n`, `@typescript-eslint/parser`) needs to be available inside the sandboxed nix build. Two ways to get it there: vendor a `package.json` + `bun.lock` + `bun.nix` under `nix/bun2nix/lint/` and let `fetchBunDeps` materialize the closure, or fetch each package as a fixed-output subderivation at build time.
...
* **Vendor + check in (`nix/bun2nix/lint/{package.json,bun.lock,bun.nix,eslint.config.js}`)** — committed lockfile, `fetchBunDeps` stages the closure, `bun install --frozen-lockfile` runs offline.
...
Chosen option: **Vendor + check in**, because it gives transitive reproducibility for free via the committed `bun.lock`, accepting four extra committed files under `nix/bun2nix/lint/` that need refreshing on bumps.
...
* Bad, because dependents who want to read the eslint version must look at `nix/bun2nix/lint/package.json` rather than a single line in `default.nix`.

### Confirmation

`nix flake check` exercises `test/nix/zx-process-exit-fail`, `zx-process-exit-disabled`, and `zx-no-process-exit`. The lint pipeline is correctly wired iff all three fixtures resolve as expected (fail / pass / pass).
...
* Neutral, because the eslint stack lives under `nix/bun2nix/lint/` rather than inline in `default.nix`.
...
* amarbel-llc/bun#7 — the issue this ADR backs.
